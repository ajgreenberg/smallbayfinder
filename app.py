...
# ---------------------------------
# Widgets (explicit keys to avoid duplicate IDs)
# ---------------------------------
...
def fetch_arcgis_geojson(layer_url: str, where: str, out_fields: str = "*", geometry: Optional[dict] = None, expect_points: bool = False) -> gpd.GeoDataFrame:
    """Fetch GeoJSON; if that fails and we're expecting points, fall back to EsriJSON->GeoDataFrame for points.
    Returns an empty GeoDataFrame on any hard error.
    """
    base_params = {
        "where": where,
    ...
    try:
        ...
        r = requests.get(layer_url.rstrip('/') + "/query", params=params, timeout=60)
        r.raise_for_status()
        gj = r.json()
    ...
    # Fallback for point layers: EsriJSON → GeoDataFrame
    if expect_points:
        try:
            features: List[dict] = []
            result_offset = 0
            ...
            # Build from esri point geometries
            xs = []
            ys = []
            attrs = []
            for f in features:
                ...
                if x is None or y is None:
                    continue
                xs.append(x); ys.append(y); attrs.append(f.get("attributes", {}))
            if not xs:
                return gpd.GeoDataFrame(columns=["geometry"], geometry=[], crs="EPSG:4326")
            df = pd.DataFrame(attrs)
            gdf = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(xs, ys), crs="EPSG:4326")
            return gdf
        except Exception:
            return gpd.GeoDataFrame(columns=["geometry"], geometry=[], crs="EPSG:4326")
...
if mode == "Auto‑fetch (no downloads)":
    ...
    if fetch_pts and parcels is not None and not parcels.empty:
        with st.spinner("Fetching Address Points…"):
            # Use expect_points=True so we fall back to EsriJSON if GeoJSON isn't supported
            addrpts = fetch_arcgis_geojson(ADDRPTS_LAYER, "1=1", out_fields="*", geometry=extent, expect_points=True)
        st.success(f"Loaded {0 if addrpts is None else len(addrpts):,} address points.")
...
if addrpts is not None and len(addrpts) > 0:
    try:
...
st.caption("Imagery: USGS The National Map (USGSImageryOnly). Parcels & Address Points: City of Milwaukee MapServer (GeoJSON/EsriJSON).")
...
with st.expander("Diagnostics"):
    st.write({
        ...
        "filters": {
            ...
            "min_addrpts_effective": min_addrpts if addrpts is not None and len(addrpts) > 0 else 0,
        },
    })
